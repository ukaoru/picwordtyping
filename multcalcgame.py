# Time-stamp: <2023-06-25 14:38:27 uchik>

#!/usr/bin/env python
# coding: utf-8

""" multcalcgame.py : number multiplying game
...
def checkans():
    ...
    if inp := st.session_state.txt:
        if inp.replace('-', '').isdigit() and int(inp) == ans:
            ...
            msg = f'Correct! {x} x {y} = {ans}.'
            msg2 = f'Correct! {x} times {y} = {ans}.'
            st.write(msg)
            speaktext(msg2)
            st.session_state.point += 1
        else:
            #msg2 = msg = f'Wrong.  {x} + {y} = {ans}, not {inp}.'
            msg = f'Wrong.  {x} x {y} = {ans}, not {inp}.'
            msg2 = f'Wrong.  {x} times {y} = {ans}, not {inp}.'
            st.write(msg)
            speaktext(msg2)
